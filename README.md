...
Work was split one pillar per person, with each of us owning the modules, tests and
documentation for our own area.

| Member | Area | Contribution |
|---|---|---|
| Linus | Pillar I: retrieval and ranking | Catalog indexing and the BM25/FTS5 search path, intent routing and RRF fusion, and the reranking cascade with its fallback rungs. |
| Tharun | Pillar II: dialogue strategy | Dialogue state and constraint accumulation, intent-override erasure, the ask-vs-answer policy, and candidate-derived question selection. |
| Jamie | Pillar III: orchestration and performance | Context distillation and runtime re-planning, the per-turn decision trace, and the device-selection path that made the GPU diagnostic runs affordable. |
| Mayukhi | Pillar IV: evaluation and delivery | The evaluation matrix with scenario breakdowns, k-fold and paired significance testing, the invariant test suite, the demo app, and the submission write-ups. |

Every change was kept or reverted on a measured run rather than on judgement, and all four of
us reviewed the experiments that shipped. The full ledger, including rejected work, is in
